...
public class Screen 
{
  ...
  public String[] displayInAscending()
  {
   Set<Double> area_tset=new TreeSet<Double>(shapeArea);
//   Set<ScreenElements> screen_set=new Set<ScreenElements>(screenElements);
   String shapeNames[]=new String[screenElements.size()];
    for(int index_1=0; index_1< area_tset.size();index_1++)
   {
    	for (int index = 0; index < screenElements.size(); index++)
	{
		if(screenElements[index]==area_tset[index_1])
       shapeNames[index_1] = screenElements.get(index).getShape().getType();
	}
   }
   checkForEmptyAndThrowException();
   return shapeNames;
